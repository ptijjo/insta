...
export class UserService {
  ...
  public async createUser(userData: CreateUserDto): Promise<User> {
    ...
    const photo = 'https://vibz.s3.eu-central-1.amazonaws.com/logo/photoProfil.png';
    const hashedPassword = await hash(userData.password, 10);
    const createUserData: User = await this.user.create({ data: { ...userData, password: hashedPassword, profil_photo: photo } });
